...
class BotAI {
  ...
  getDifficultyParams() {
    const params = {
      easy: { actionInterval: 3000, aggressiveness: 0.2, expansionRate: 0.3, troopEfficiency: 0.6, allianceProbability: 0.1, mistakeRate: 0.3 },
      medium: { actionInterval: 2000, aggressiveness: 0.4, expansionRate: 0.5, troopEfficiency: 0.8, allianceProbability: 0.3, mistakeRate: 0.15 },
      hard: { actionInterval: 1000, aggressiveness: 0.6, expansionRate: 0.7, troopEfficiency: 0.95, allianceProbability: 0.5, mistakeRate: 0.05 },
      insane: { actionInterval: 500, aggressiveness: 0.8, expansionRate: 0.9, troopEfficiency: 1.0, allianceProbability: 0.7, mistakeRate: 0 }
    };
  ...
  detectThreats(myTerritories) {
    ...
    myTerritories.forEach(({ x, y, cell }) => {
      ...
      neighbors.forEach(neighbor => {
        if (neighbor.cell.owner && neighbor.cell.owner !== this.bot.id && !this.bot.isAlliedWith(neighbor.cell.owner)) {
    ...
    myTerritories.forEach(({ x, y }) => {
      ...
      neighbors.forEach(neighbor => {
        ...
        if (!visited.has(key) && neighbor.cell.type === 'land') {
          ...
          if (opportunity.value > 0) opportunities.push(opportunity);
        }
      });
    });
  ...
  calculateMilitaryStrength() {
    ...
    territories.forEach(({ x, y, cell }) => {
      ...
      if (this.isBorderTerritory(x, y)) borderTroops += cell.troops;
    });
  ...
  isBorderTerritory(x, y) {
    const neighbors = this.gameRoom.mapGrid.getNeighbors(x, y);
    return neighbors.some(n => n.cell.type === 'land' && n.cell.owner !== this.bot.id);
  }
  ...
  selectStrategy(situation) {
    if (situation.threats.length > 3 && situation.militaryStrength.ratio < 0.3) return 'defensive';
    if (situation.controlPercentage > 0.3 && situation.gold > 2000) return 'aggressive';
    if (situation.myTerritories.length < 10) return 'economic';
    if (situation.opportunities.length > 5) return 'expansion';
    if (situation.playerAnalysis.length > 2 && Math.random() < this.params.allianceProbability) return 'diplomatic';
    return 'balanced';
  }

  async executeStrategy(strategy, situation) {
    if (Math.random() < this.params.mistakeRate) strategy = 'random';
    
    switch (strategy) {
      case 'defensive': await this.executeDefensiveStrategy(situation); break;
      case 'aggressive': await this.executeAggressiveStrategy(situation); break;
      case 'economic': await this.executeEconomicStrategy(situation); break;
      case 'expansion': await this.executeExpansionStrategy(situation); break;
      case 'diplomatic': await this.executeDiplomaticStrategy(situation); break;
      case 'balanced': await this.executeBalancedStrategy(situation); break;
      case 'random': await this.executeRandomAction(situation); break;
    }
  }
  ...
  async executeAggressiveStrategy(situation) {
    const weakestPlayer = situation.playerAnalysis.find(p => !p.isAlly && p.strength < 0.5);
    if (weakestPlayer) {
  ...
  evaluateCitySpot(x, y) {
    ...
    if (!this.isBorderTerritory(x, y)) score += 20;
    const buildingNearby = neighbors.some(n => n.cell.building);
    if (buildingNearby) score -= 15;
    return score;
  }
  ...
  findBestBarracksSpot() {
    const borderTerritories = this.findBorderTerritories().filter(t => !t.cell.building);
  ...
  findBuildingSpot(buildingType) {
    const territories = this.gameRoom.mapGrid.getPlayerCells(this.bot.id).filter(t => !t.cell.building);
    ...
    if (buildingType === 'outpost' || buildingType === 'barracks') {
      ...
      if (border.length > 0) return border[Math.floor(Math.random() * border.length)];
    }
    if (territories.length > 0) return territories[Math.floor(Math.random() * territories.length)];
    return null;
  }
  ...
  countAllLandCells() {
    ...
    for (let y = 0; y < this.gameRoom.mapGrid.height; y++) {
      for (let x = 0; x < this.gameRoom.mapGrid.width; x++) {
        if (this.gameRoom.mapGrid.grid[y][x].type === 'land') count++;
      }
    }
    return count;
  }
